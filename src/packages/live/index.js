...
const baseUrl = "https://live.shopee.ph";
...
const getChatroom = ({ token }) => {
  ...
  return axios
    .get(
      `https://chatroom-live.shopee.ph/api/v1/fetch/chatroom/${id}/message?${qs.stringify(
        query
      )}`,
      { headers: { Cookie: cookies.join(";") } }
    )
...
const connectWs = ({ sessId, uid, deviceId, userSig }) => {
  ...
  return axios
    .get(`https://live.shopee.ph/im/v1/comet?${qs.stringify(query)}`, {
      headers: {
